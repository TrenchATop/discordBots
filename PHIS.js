...
const payLoad = {"search":"","types":["listing","offer"],"project":"The Pixel Head Squad - Accessories","sort":{"_id":-1},"priceMin":null,"priceMax":null,"page":1,"verified":true,"nsfw":false,"sold":true,"smartContract":false}
...
function gettingTime(time){
  ...
  .then(response => {
      let sold = response.data.results
      .filter(data => gettingTime(data.createdAt) >= (new Date().getTime()- 10000));
      (sold.length) ? discordListing(sold):"no";
  })
  ...
  function discordListing(data) {
    console.log("Sold Pixel Head Item");
    ...
    for (let i = 0; i < (data.length); i++) {
        ...
        const exampleEmbed = new MessageEmbed()
            ...
            .addFields(
                ...
                { name: "On the market for: ", value: smartContract, inline: true }
            )
            .setImage(image)
            .setTimestamp()
...
client.once('ready', () => {
    console.log("PHIS bot is running");
    listings();
});
